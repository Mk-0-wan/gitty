...
class DBStorage:
    ...
    def acc_authentication(self, email, password):
        """Authenticates the users who are already in the database"""
        from gitty_v1.accounts.account import Accounts

        # Query the database for the user with the provided email
        account_user = self.__session.query(Accounts).filter_by(email=email).first()

        print(account_user)
        if account_user and bcrypt.checkpw(password.encode('utf-8'), account_user.password.encode('utf-8')):
            return account_user
        else:
            return None
